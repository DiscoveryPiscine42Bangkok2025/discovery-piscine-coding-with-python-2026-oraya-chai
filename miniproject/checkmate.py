...
def checkmate(board_str=None):
    ...
    # 2 ต้องเป็นstringเท่านั้น เผื่อมีการใส่เลข
    if not isinstance(board_str, str):
        print("Error")
        return 
    
    rows = board_str.splitlines() 
    # ['', 'R...', '.K..', '..P.', '....'] โดย '' มาจาก newlineตัวแรกในtripple quoteเพราะมีการกดขึ้นบรรทัดใหม่เลยเป็นงั้น
    # ไม่มีที่ตัวสุดท้ายเพราะ splitlines() ไม่สร้างสมาชิกว่างจากnewlineตัวสุดท้าย
    rows = [row for row in rows if row.strip() != ""]
    # ['R...', '.K..', '..P.', '....']
    
    # 3 กรณีกระดานว่าง แบบ board = " "
    if len(rows) == 0:
        print("Error") 
        return

    # 4 เช็กว่าเป็นสี่เหลี่ยมจตุรัสมั้ย
    n = len(rows)
    for row in rows:
        if len(row) != n:
            print("Error")
    ...
    # 5 หา King
    king_position = None # ไว้เก็บตำแหน่งคิงในรูปของ(row, col)เริ่มต้นเป็นnoneเพราะยังไม่เจอking
    ...
    for i in range(n):
        for j in range(n):
            piece = rows[i][j]
            # คัดเฉพาะหมากที่โจมตีได้ ถ้าไม่ใช่หมากที่โจมตีได้ ก็ให้ข้ามไปช่องถัดไป
            ...
            # PAWN
                ...
                # ตรวจเส้นทางเดินpawn -> ให้มีการเช็กทั้ง2ฝั่ง 
            ...
            for dr, dc in directions:
                ...
                while in_bounds(ro, co):
                    if rows[ro][co] == "K":
                        ...
                        print("Success")
                        return
                    # เจอหมากตัวอื่นให้หยุด -> หมากกินข้ามตัวอื่นไม่ได้
